...
- **Text-only chat history** — clientStorage gives the plugin 5MB across all keys while one attached image may be 26MB, so saved chats keep the text and drop the image payloads; the bubble notes how many were dropped. A failed write now raises a Figma notification instead of a status line that only renders on an empty chat screen.
- **Move chats between projects** — drag a chat onto another project's heading, or use its "Move to…" select, to reassign which document it is grouped under. The panel is 320-680px wide, so both are offered since the sticky heading alone is an easy miss to drop onto.
